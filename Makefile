...
generate_coverage_report:
	docker exec -it $(ODOO_CONTAINER) coverage run /usr/bin/odoo -d db_test --test-enable -p 8001 --stop-after-init --log-level=test
...
init_test_db:
	...
	docker exec -t $(ODOO_CONTAINER) $(ODOO_BIN) -c /opt/odoo/odoo.conf -i all -d db_test --stop-after-init
	docker exec -t $(ODOO_CONTAINER) $(ODOO_BIN) -c /opt/odoo/odoo.conf -i plus_poc -d db_test --stop-after-init
